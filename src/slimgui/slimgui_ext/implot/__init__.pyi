...
def colormap_button(label: str, size: tuple[float, float] = (0,0), cmap: Colormap | int = IMPLOT_AUTO) -> bool: ...

def colormap_scale(label: str, scale_min: float, scale_max: float, size: tuple[float, float] = (0,0), format: str = %g, flags: ColormapScaleFlags = ColormapScaleFlags.NONE, cmap: Colormap | int = IMPLOT_AUTO) -> None: ...
...
def set_next_marker_style(marker: Marker | int = IMPLOT_AUTO, size: float = AUTO, fill: tuple[float, float, float, float] = AUTO_COL, weight: float = AUTO, outline: tuple[float, float, float, float] = AUTO_COL) -> None: ...

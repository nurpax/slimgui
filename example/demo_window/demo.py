...
import slimgui
from slimgui import imgui, BoolRef, IntRef
...
def show_demo_window(show_window: slimgui.BoolRef, texture: dict[str, Any]):
    ...
    if st.show_tool_metrics:
        imgui.show_metrics_window(st.show_tool_metrics)

    if st.show_tool_debug_log:
        imgui.show_debug_log_window(st.show_tool_debug_log)

    if st.show_tool_id_stack_tool:
        imgui.show_id_stack_tool_window(st.show_tool_id_stack_tool)

    if st.show_tool_style_editor:
        imgui.begin("Dear ImGui Style Editor", st.show_tool_style_editor)
        imgui.show_style_editor()
        imgui.end()

    if st.show_tool_about:
        imgui.show_about_window(st.show_tool_about)

    if st.show_native_imgui_demo_window:
        imgui.show_demo_window(st.show_native_imgui_demo_window)
    ...
    if not imgui.begin("Python mini-port of Dear ImGui Demo", open=open_ref, flags=window_flags):
        imgui.end()
        return
    ...
    if imgui.begin_menu_bar():
        ...
        if imgui.begin_menu("Examples"):
            imgui.menu_item("Main menu bar", shortcut=None, selected=st.show_app_main_menu_bar)
            imgui.separator_text("Mini apps")
            imgui.menu_item("Console", shortcut=None, selected=st.show_app_console)
            imgui.menu_item("Custom rendering", shortcut=None, selected=st.show_app_custom_rendering)
            imgui.menu_item("Documents", shortcut=None, selected=st.show_app_documents)
            imgui.menu_item("Log", shortcut=None, selected=st.show_app_log)
            imgui.menu_item("Property editor", shortcut=None, selected=st.show_app_property_editor)
            imgui.menu_item("Simple layout", shortcut=None, selected=st.show_app_layout)
            imgui.menu_item("Simple overlay", shortcut=None, selected=st.show_app_simple_overlay)

            imgui.separator_text("Concepts")
            imgui.menu_item("Auto-resizing window", shortcut=None, selected=st.show_app_auto_resize)
            imgui.menu_item("Constrained-resizing window", None, selected=st.show_app_constrained_resize)
            imgui.menu_item("Fullscreen window", None, selected=st.show_app_fullscreen)
            imgui.menu_item("Manipulating window titles", None, selected=st.show_app_window_titles)
            imgui.end_menu()

        if imgui.begin_menu("Tools"):
            imgui.menu_item("Metrics/Debugger", None, selected=st.show_tool_metrics)
            imgui.menu_item("Debug Log", None, selected=st.show_tool_debug_log)
            imgui.menu_item("ID Stack Tool", None, selected=st.show_tool_id_stack_tool)
            imgui.menu_item("Style Editor", None, selected=st.show_tool_style_editor)
            imgui.menu_item("About Dear ImGui", None, selected=st.show_tool_about)
            imgui.separator()
            imgui.menu_item("ImGui Native Demo Window", None, selected=st.show_native_imgui_demo_window)
            imgui.end_menu()
        imgui.end_menu_bar()
...
#     }
    ...
    if imgui.collapsing_header("Configuration"):
        ...
        if imgui.tree_node("Configuration##2"):
            ...
            flags_ref = IntRef(io.config_flags)
            _ = imgui.checkbox_flags("io.ConfigFlags: NAV_ENABLE_KEYBOARD", flags_ref, imgui.ConfigFlags.NAV_ENABLE_KEYBOARD)
            imgui.same_line(); help_marker("Enable keyboard controls.")
            _ = imgui.checkbox_flags("io.ConfigFlags: NAV_ENABLE_GAMEPAD", flags_ref, imgui.ConfigFlags.NAV_ENABLE_GAMEPAD)
            ...
            _ = imgui.checkbox_flags("io.ConfigFlags: NO_MOUSE", flags_ref, imgui.ConfigFlags.NO_MOUSE)
            if (flags_ref.value & imgui.ConfigFlags.NO_MOUSE) != 0:
                if math.fmod(imgui.get_time(), 0.40) < 0.2:
                    imgui.same_line()
                    imgui.text("<<PRESS SPACE TO DISABLE>>")
                    if imgui.is_key_pressed(imgui.Key.KEY_SPACE):
                        flags_ref.value &= ~imgui.ConfigFlags.NO_MOUSE
            _ = imgui.checkbox_flags("io.ConfigFlags: NO_MOUSE_CURSOR_CHANGE", flags_ref, imgui.ConfigFlags.NO_MOUSE_CURSOR_CHANGE)
            io.config_flags = imgui.ConfigFlags(int(flags_ref))
            ...
            def checkbox(label: str, value: bool):
                ref = BoolRef(value)
                _ = imgui.checkbox(label, ref)
                return ref.value

            io.config_input_trickle_event_queue = checkbox("io.config_input_trickle_event_queue", io.config_input_trickle_event_queue)
            ...
            io.mouse_draw_cursor = checkbox("io.mouse_draw_cursor", io.mouse_draw_cursor)
            ...
            io.config_input_text_cursor_blink = checkbox("io.config_input_text_cursor_blink", io.config_input_text_cursor_blink)
            imgui.same_line(); help_marker("Enable blinking cursor (optional as some users consider it to be distracting).")
            io.config_input_text_enter_keep_active = checkbox("io.config_input_text_enter_keep_active", io.config_input_text_enter_keep_active)
            imgui.same_line(); help_marker("Pressing Enter will keep item active and select contents (single-line only).")
            io.config_drag_click_to_input_text = checkbox("io.config_drag_click_to_input_text", io.config_drag_click_to_input_text)
            ...
            io.config_windows_resize_from_edges = checkbox("io.config_windows_resize_from_edges", io.config_windows_resize_from_edges)
            ...
            io.config_windows_move_from_title_bar_only = checkbox("io.config_windows_move_from_title_bar_only", io.config_windows_move_from_title_bar_only)
            io.config_mac_osx_behaviors = checkbox("io.config_mac_osx_behaviors", io.config_mac_osx_behaviors)
        ...
        if imgui.tree_node("Backend Flags"):
            ...
            backend_flags_int = IntRef(io.config_flags)
            imgui.checkbox_flags("io.BackendFlags: HAS_GAMEPAD", backend_flags_int, imgui.BackendFlags.HAS_GAMEPAD)
            imgui.checkbox_flags("io.BackendFlags: HAS_MOUSE_CURSORS", backend_flags_int, imgui.BackendFlags.HAS_MOUSE_CURSORS)
            imgui.checkbox_flags("io.BackendFlags: HAS_SET_MOUSE_POS", backend_flags_int, imgui.BackendFlags.HAS_SET_MOUSE_POS)
            imgui.checkbox_flags("io.BackendFlags: RENDERER_HAS_VTX_OFFSET", backend_flags_int, imgui.BackendFlags.RENDERER_HAS_VTX_OFFSET)
            io.backend_flags = imgui.BackendFlags(backend_flags_int.value)
    ...
    if imgui.collapsing_header("Window options"):
        if imgui.begin_table("split", 3):
            ...
            imgui.checkbox("No titlebar", st.no_titlebar)
            imgui.table_next_column()
            imgui.checkbox("No scrollbar", st.no_scrollbar)
            imgui.table_next_column()
            imgui.checkbox("No menu", st.no_menu)
            imgui.table_next_column()
            imgui.checkbox("No move", st.no_move)
            imgui.table_next_column()
            imgui.checkbox("No resize", st.no_resize)
            imgui.table_next_column()
            imgui.checkbox("No collapse", st.no_collapse)
            imgui.table_next_column()
            imgui.checkbox("No close", st.no_close)
            imgui.table_next_column()
            imgui.checkbox("No nav", st.no_nav)
            imgui.table_next_column()
            imgui.checkbox("No background", st.no_background)
            imgui.table_next_column()
            imgui.checkbox("No bring to front", st.no_bring_to_front)
            imgui.table_next_column()
            imgui.checkbox("No docking", st.no_docking)
            imgui.table_next_column()
            imgui.checkbox("Unsaved document", st.unsaved_document)
            imgui.end_table()
...
def show_example_app_simple_overlay(st: State):
    ...
    if imgui.begin("Example: Simple overlay", st.show_app_simple_overlay, window_flags):
        ...
        if imgui.begin_popup_context_window():
            if imgui.menu_item('Custom', None, _location == -1):
                _location = -1
            if imgui.menu_item('Center', None, _location == -2):
                _location = -2
            if imgui.menu_item('Top-left', None, _location == 0):
                _location = 0
            if imgui.menu_item('Top-right', None, _location == 1):
                _location = 1
            if imgui.menu_item('Bottom-left', None, _location == 2):
                _location = 2
            if imgui.menu_item('Bottom-right', None, _location == 3):
                _location = 3
            # TODO close
            imgui.end_popup()
        imgui.end()
